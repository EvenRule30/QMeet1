...
import { CalendarEvent, MemoryTask, Note, RecentAction, SearchResponse } from '../types';
import {
...
export function useMemoryContext({
  ...
  const handleExportMemory = useCallback(async () => {
    try {
      ...
      downloadJsonFile(payload, `qmeet-memory-${new Date().toISOString().slice(0, 10)}.json`);
      setMemorySyncState('synced');
      ...
      downloadJsonFile(payload, `qmeet-memory-local-${new Date().toISOString().slice(0, 10)}.json`);
      setMemorySyncState('error');
  ...
  const handleImportMemoryFile = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
