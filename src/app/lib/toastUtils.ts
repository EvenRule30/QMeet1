...
export type ResultToastKind = 'success' | 'info' | 'warning' | 'error' | 'search' | 'calendar' | 'notes';
...
export function getResultToastForCommand(command: string, fullText: string): Omit<ResultToast, 'id' | 'createdAt'> | null {
  const trimmed = fullText.trim();
