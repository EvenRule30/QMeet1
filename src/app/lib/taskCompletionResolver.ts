...
function scoreCandidate(queryTokens: string[], task: MemoryTask): number {
  ...
  const taskCoverage = taskTokens.filter((token) => querySet.has(token)).length / taskTokens.length;
...
export function resolveGlobalTaskCompletionReference(
  ...
  if (exact.length > 1) return { kind: 'ambiguous', candidates: exact.slice(0, 5) };
