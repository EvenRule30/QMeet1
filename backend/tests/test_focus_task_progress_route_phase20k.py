...
class FocusTaskProgressRoutePhase20KTests(unittest.TestCase):
    ...
    def test_confirmed_linked_task_completion_uses_verified_bridge(self) -> None:
        ...
        local_commit_index = self.app_source.index(
            "completeConfirmedTaskTargets(",
            bridge_index,
        )
        memory_handler_index = self.app_source.index(
            "await handleMemoryCommand(",
            local_commit_index,
        )
        self.assertLess(bridge_index, local_commit_index)
        self.assertLess(local_commit_index, memory_handler_index)
    ...
    def test_frontend_claims_progress_only_after_verified_response(self) -> None:
        ...
        failure_return_index = self.app_source.index(
            "I could not verify the linked Focus task completion, so no task was changed.",
            bridge_index,
        )
        self.assertLess(bridge_index, failure_return_index)
        self.assertLess(failure_return_index, local_commit_index)
    ...
    def test_backend_route_never_returns_an_unverified_success(self) -> None:
        ...
        self.assertIn(
            "record_focus_task_progress_verified(request)",
            self.router_source,
        )
    ...
    def test_bridge_preserves_lock_order_and_rollback_guarantee(self) -> None:
        ...
        relationship_lock = self.bridge_source.index(
            "with _RELATIONSHIP_LOCK",
            focus_lock,
        )
        memory_lock = self.bridge_source.index(
    ...
    def test_bridge_does_not_auto_complete_focus(self) -> None:
        ...
        self.assertIn(
            "complete the Focus when ready",
            self.bridge_source,
        )
        self.assertIn(
            "focusContinuityPreserved",
            self.bridge_source,
        )
