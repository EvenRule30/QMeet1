...
from app.qmeet_agent_shadow import AGENT_SHADOW_SYSTEM_PROMPT, GLOBAL_CAPABILITY_CONTRACT
...
RESOLVER = (ROOT / "src" / "app" / "lib" / "taskCompletionResolver.ts").read_text(encoding="utf-8")
...
class AgentTaskCompletionPromotionPhase21C4Tests(unittest.TestCase):
    ...
    def test_agent_prompt_promotes_one_semantic_completion_reference(self) -> None:
        self.assertIn("proposedAction=mark-task-done", AGENT_SHADOW_SYSTEM_PROMPT)
        self.assertIn('{"scope": "global", "query": "<concise identifying task reference>"}', AGENT_SHADOW_SYSTEM_PROMPT)
        ...
        self.assertIn("multiple candidates require clarification", AGENT_SHADOW_SYSTEM_PROMPT)

    def test_frontend_validator_requires_global_scope_and_query_only(self) -> None:
        self.assertIn("export function isPromotedTaskCompletionToolDecision", PROMOTION)
        self.assertIn("export function resolvePromotedTaskCompletionToolCommand", PROMOTION)
        self.assertIn("hasExactlyKeys(argumentsValue, ['scope', 'query'])", PROMOTION)
        self.assertIn("argumentsValue.scope !== 'global'", PROMOTION)
        self.assertIn("decision.proposedAction === 'mark-task-done'", PROMOTION)
    ...
    def test_promoted_no_match_retries_authoritative_task_state(self) -> None:
        ...
        local_resolution = APP.index("let effectiveTaskCompletionResolution =")
        refresh = APP.index("const authoritativeTasks = await getMemoryTasks();", local_resolution)
        no_match_reply = APP.index("I couldn't find an open task matching", refresh)
        ...
        self.assertIn("resolveGlobalTaskCompletionReference", refreshed_source)
    ...
    def test_resolved_task_identity_reuses_existing_confirmation_seam(self) -> None:
        self.assertIn("{ id: resolvedTask.id, title: resolvedTask.title }", APP)
        self.assertIn("promotedTaskCompletionTarget: ConfirmedTaskTarget | null = null", APP)
        self.assertIn("task.id === promotedTaskCompletionTarget.id", APP)
        self.assertIn("task.title.trim() === promotedTaskCompletionTarget.title.trim()", APP)
        self.assertIn("id: promotedTaskCompletionTarget.id", APP)
        self.assertIn("title: promotedTaskCompletionTarget.title", APP)
        self.assertIn("pendingTaskCompletionTargetsRef.current = isTaskCompletionCommand", APP)
        self.assertIn("const confirmedTaskCommandMatch: CommandMatch | undefined =", APP)
        self.assertIn("confirmedTaskCommandMatch,\n              resolvedTaskTargets", APP)

    def test_confirm_rechecks_identity_against_authoritative_tasks_if_local_state_is_stale(self) -> None:
        immutable_local = APP.index("const immutableConfirmedTaskTargets =")
        execution_state = APP.index("let confirmedTaskExecutionState = memoryTasks;", immutable_local)
        refresh = APP.index("const authoritativeTasks = await getMemoryTasks();", execution_state)
        immutable_guard = APP.index("setTrackedInputRoute('Confirmed task identity changed')", refresh)
        ...
        self.assertIn("authoritativeTasks.tasks ?? []", refreshed_source)
        self.assertIn("confirmedTaskExecutionState = authoritativeTaskState", refreshed_source)
        self.assertIn("verifiedConfirmedTaskTargets = authoritativeConfirmedTaskTargets", refreshed_source)

    def test_single_task_resolver_has_zero_one_many_outcomes(self) -> None:
        for expected in ("kind: 'exact'", "kind: 'likely'", "kind: 'ambiguous'", "kind: 'none'"):
            self.assertIn(expected, RESOLVER)
        self.assertIn("tasks.filter((task) => !task.completedAt)", RESOLVER)
        self.assertIn("queryTokens.length === 1", RESOLVER)
        self.assertIn("Math.abs(best.score - runnerUp.score) < 0.15", RESOLVER)
