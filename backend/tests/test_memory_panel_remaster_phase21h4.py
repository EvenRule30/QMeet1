...
class MemoryPanelRemasterPhase21H4Tests(unittest.TestCase):
    ...
    def test_everyday_surface_prioritizes_focus_and_tasks(self):
        # Phase 21I3B makes the Focus/Tasks className dynamic so it can append
        # a temporary context-emphasis class. Preserve the original H4 intent
        # without requiring the retired static className literal.
        self.assertIn("memory-remaster-card memory-remaster-focus-card", self.source)
        self.assertIn("memory-remaster-task-input-row", self.source)
    ...
    def test_i3b_context_emphasis_does_not_replace_remaster_structure(self):
        self.assertIn("memory-remaster-context-target", self.source)
        self.assertIn(".memory-remaster-context-target", self.css)
        self.assertIn("@keyframes memory-remaster-context-pulse", self.css)
