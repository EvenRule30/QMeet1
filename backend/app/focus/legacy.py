...
from app.focus.models import FocusStatus, LegacyFocusSeed
...
def load_legacy_focus_seed() -> LegacyFocusSeed | None:
    ...
    context_payload = _call_optional(
        "app.work_context", "get_background_work_context"
    )
    context = (
    ...
    title = _clean(session.get("title"), 180) or _clean(context.get("title"), 180)
    if not title:
        return None
    ...
    mode = _clean(context.get("mode"), 60) or _clean(session.get("mode"), 60)
    ...
    return LegacyFocusSeed(
        ...
        milestones=_clean_list(context.get("openQuestions"), 20),
        completedMilestones=_clean_list(context.get("recentProgress"), 40),
